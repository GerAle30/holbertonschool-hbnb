...
def test_review_creation_validation():
    ...
    print("[TEST] Testing Create Review Validation Logic\n")
    ...
    if response.status_code != 200:
        print(f"[FAIL] Failed to login: {response.text}")
        return
    ...
    if response.status_code != 201:
        print(f"[FAIL] Failed to create second user: {response.text}")
        return
    
    user2_id = response.json()['id']
    print(f"[SUCCESS] Second user created with ID: {user2_id}")
    ...
    if response.status_code != 200:
        print(f"[FAIL] Failed to login as second user: {response.text}")
        return
    
    token2 = response.json()['access_token']
    headers2 = {"Authorization": f"Bearer {token2}"}
    print("[SUCCESS] Second user login successful")
