...
def test_delete_review_validation():
    ...
    print("🧪 Testing Delete Review Validation Logic\n")
    ...
    if response.status_code != 201:
        print(f"❌ Failed to create second place: {response.text}")
        return
    ...
    if response.status_code != 201:
        print(f"❌ Failed to create second review: {response.text}")
        return
    
    review2_id = response.json()['id']
    print(f"✅ Second review created with ID: {review2_id}")
